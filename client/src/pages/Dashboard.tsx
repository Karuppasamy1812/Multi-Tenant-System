...
import { FolderKanban, Users, CheckSquare, Loader2, Trash2, Plus } from 'lucide-react';
import { toast } from 'sonner';
import * as Dialog from '@radix-ui/react-dialog';
import { useState, type FormEvent } from 'react';
import { useProjects, useCreateProject, useArchiveProject } from '../queries/projects.query';
...
export default function Dashboard() {
  ...
  const handleCreate = (e: FormEvent) => {
    ...
    createProject.mutate({ name, description }, {
      onSuccess: () => { toast.success('Project created'); setOpen(false); setName(''); setDescription(''); },
    });
  };
    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          <span className="font-medium text-indigo-600">{user?.tenant?.name}</span> workspace
        </p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { label: 'Total Projects', value: stats.projects, icon: FolderKanban, color: 'text-indigo-600 bg-indigo-50' },
            { label: 'Team Members',   value: stats.members,  icon: Users,        color: 'text-violet-600 bg-violet-50' },
            { label: 'Total Tasks',    value: stats.tasks,    icon: CheckSquare,  color: 'text-emerald-600 bg-emerald-50' },
          ].map(s => (
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-dashed border-gray-200 py-20 text-center">
          ...
          {canCreate && <p className="text-sm text-gray-400 mt-1">Click "New Project" to get started</p>}
        </div>
      ) : (
          ...
          {projects.map((p) => {
            ...
            return (
              <div key={p._id}
                ...
                {canCreate && (
                  <button onClick={e => { e.stopPropagation(); archiveProject.mutate(p._id, { onSuccess: () => toast.success('Project archived') }); }}
                ...
                <h2 className="font-semibold text-gray-900 group-hover:text-indigo-600 transition pr-6 truncate">{p.name}</h2>
                <p className="text-sm text-gray-400 mt-1 line-clamp-2 min-h-[40px]">{p.description || 'No description'}</p>
